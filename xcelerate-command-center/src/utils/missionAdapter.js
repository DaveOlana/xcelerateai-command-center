...
// following the standard also works, with no rename of any existing data.
//
// Singular-string handling: the new JSON authoring standard authors
// `stretchChallenge` as a plain string (not an array). firstArray() only
// matches non-empty arrays, so a string value was silently dropped before.
// The fix wraps string candidates into a 1-element array — the same
// singular/plural dual-acceptance pattern resolveCommanderNotes() already
// uses for commanderNote singular strings.
function resolveStretchGoals(mission, rawWeek) {
  // Check array candidates first (highest priority, preserves existing behaviour)
  ...
  // Singular string fallback — new authoring standard uses a plain string
  for (const c of [mission?.stretchChallenge, rawWeek?.stretchChallenge, mission?.stretchGoal]) {
