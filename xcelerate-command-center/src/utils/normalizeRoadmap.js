...
function resolveSkillCheck(item) {
  ...
  // Flatten nested skill-check containers (enriched JSON format).
  // Some schemas wrap questions inside a container object:
  //   { skillCheckId, title, instructions, questions: [{prompt, answerType, options}], readyWhen: [...] }
  // Detect these by the presence of a non-empty inner `questions` array and expand
  // each container's questions into the output, carrying readyWhen/instructions
  // through for optional UI display. Plain question objects and strings pass through
  // the else-branch unchanged, preserving all existing behaviour.
  const flattened = [];
  for (const q of raw) {
    if (typeof q === 'object' && q !== null && Array.isArray(q.questions) && q.questions.length > 0) {
      // Skill-check container — extract its inner questions
      const readyWhen    = Array.isArray(q.readyWhen) ? q.readyWhen : [];
      const instructions = typeof q.instructions === 'string' ? q.instructions : null;
      for (const innerQ of q.questions) {
        if (typeof innerQ === 'string') {
          flattened.push({
            question: innerQ,
            ...(readyWhen.length > 0 ? { readyWhen } : {}),
            ...(instructions         ? { instructions } : {}),
          });
        } else if (typeof innerQ === 'object' && innerQ !== null) {
          flattened.push({
            ...innerQ,
            // Normalise prompt → question so all downstream consumers can use one key
            question: innerQ.prompt || innerQ.question || innerQ.text || '',
            ...(readyWhen.length > 0 ? { readyWhen } : {}),
            ...(instructions         ? { instructions } : {}),
          });
        }
      ...
      // Existing formats: plain string, direct question object, or other object
      if (typeof q === 'string')                    flattened.push({ question: q });
      else if (typeof q === 'object' && q !== null) flattened.push(q);
      else                                          flattened.push({ question: String(q) });
    }
  }
  return flattened;
}
