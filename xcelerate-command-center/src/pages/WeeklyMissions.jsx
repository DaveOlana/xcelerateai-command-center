...
export default function WeeklyMissions() {
  ...
  const [skillAnsMap, setSkillAnsMap] = useState({});
  ...
  React.useEffect(() => {
    ...
    const savedAns = savedCheck?.answers;
    if (typeof savedAns === 'object' && savedAns !== null) {
      setSkillAnsMap(savedAns);
    } else if (typeof savedAns === 'string' && savedAns) {
      setSkillAnsMap({ 0: savedAns, explanation: savedAns });
    } else {
      setSkillAnsMap({});
    }
  ...
  const handleSaveSkillCheck = (e) => {
    e.preventDefault();
    const explanationStr = Object.values(skillAnsMap).filter(v => typeof v === 'string' && v.trim()).join(' | ');
    submitSkillCheck(
      week.weekNumber,
      { ...skillAnsMap, explanation: explanationStr || 'Self-assessed readiness' },
      skillConf,
      true
    );
                      ...
                      {skillCheckQuestions.length === 0 ? (
                        ...
                        <form onSubmit={handleSaveSkillCheck} className="space-y-5">
                          ...
                          {skillCheckQuestions.filter(Boolean).map((q, qi) => {
                            ...
                            const currentVal = skillAnsMap[qi] !== undefined ? skillAnsMap[qi] : (qi === 0 ? (skillAnsMap.explanation || '') : '');

                            const handleQAnsChange = (val) => {
                              setSkillAnsMap(prev => ({ ...prev, [qi]: val }));
                            };
                            ...
                            return (
                              <div key={qi} className="p-4 bg-navy-850/50 border border-navy-750 rounded-2xl space-y-3">
                                ...
                                {(answerType === 'multiple_choice' || answerType === 'select') && qOptions.length > 0 ? (
                                      ...
                                      return (
                                        <label key={oi} className="flex items-center gap-2.5 text-xs text-slate-350 cursor-pointer hover:text-white transition-colors">
                                          <input
                                            ...
                                            name={`skill-q-${qi}`}
                                            value={optValue}
                                            checked={currentVal === optValue}
                                            onChange={(e) => handleQAnsChange(e.target.value)}
                                ...
                                ) : answerType === 'short_text' || answerType === 'input' ? (
                                  <input
                                    type="text"
                                    value={currentVal}
                                    onChange={(e) => handleQAnsChange(e.target.value)}
