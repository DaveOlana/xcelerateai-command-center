                  ...
                  >
                    <div className="min-w-0 pr-2">
                      <div className="flex items-center gap-1.5">
                        <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Week {w.weekNumber}</span>
                        {!requiresProof && (
            ...
            >
              <form onSubmit={handleSubmit} className="space-y-5">
                <div className="grid md:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-4">
                    <div className="space-y-1">
                      <label className="text-[12px] font-bold text-slate-400 uppercase tracking-wider flex items-center gap-1.5">
                        ...
                        {repo && (
                          <a href={repo} target="_blank" rel="noopener noreferrer" className="text-brand-cyan hover:underline inline-flex items-center gap-0.5">
                    ...
                    <div className="space-y-1">
                      <label className="text-[12px] font-bold text-slate-400 uppercase tracking-wider flex items-center gap-1.5">
                        GitHub Commit URL *
                        {commit && (
                          <a href={commit} target="_blank" rel="noopener noreferrer" className="text-brand-cyan hover:underline inline-flex items-center gap-0.5">
                            <ExternalLink className="w-3 h-3" />
                          </a>
                        )}
                      </label>
                    ...
                    <div className="space-y-1">
                      <label className="text-[12px] font-bold text-slate-400 uppercase tracking-wider flex items-center gap-1.5">
                        Demo URL / Video Link (Optional)
                        {video && (
                          <a href={video} target="_blank" rel="noopener noreferrer" className="text-brand-cyan hover:underline inline-flex items-center gap-0.5">
                            <ExternalLink className="w-3 h-3" />
                          </a>
                        )}
                      </label>
                  ...
                  <div className="space-y-4">
                    <div className="space-y-1">
                      <label className="text-[12px] font-bold text-slate-400 uppercase tracking-wider">Screenshot / Local File Path</label>
                      <input
                        type="text"
                        placeholder="e.g. ~/Desktop/build-dashboard.png"
                        value={screenshot}
                    ...
                    <div className="space-y-2 pt-1">
                      <span className="text-[12px] font-bold text-slate-400 uppercase tracking-wider block">Verification Criteria</span>
                      <button
                        type="button"
                        onClick={() => setReadme(!readme)}
                        className={`w-full flex items-center gap-3 p-3 rounded-xl border text-left text-xs font-bold transition-all ${
                          readme
                            ? 'bg-accent-primary/10 border-accent-primary/25 text-white'
                            : 'bg-navy-800 border-navy-700/40 text-slate-400 hover:text-white'
                        }`}
                      >
                        <div className={`w-4 h-4 rounded border flex items-center justify-center flex-shrink-0 ${readme ? 'border-accent-primary bg-accent-primary/20' : 'border-navy-500'}`}>
                          {readme && <CheckCircle2 className="w-3 h-3 text-accent-primary" />}
                        </div>
                        <span>README complete with description</span>
                      </button>
                      ...
                      >
                        <div className={`w-4 h-4 rounded border flex items-center justify-center flex-shrink-0 ${reflection ? 'border-accent-primary bg-accent-primary/20' : 'border-navy-500'}`}>
                          {reflection && <CheckCircle2 className="w-3 h-3 text-accent-primary" />}
                        </div>
                        <span>Reflection details logged in journal</span>
                      </button>
                    </div>
                  </div>
                </div>
                ...
                <div className={`rounded-xl p-4 border flex items-start gap-3 transition-all ${
                  ...
                  {isProofComplete({ githubRepoLink: repo, githubCommitLink: commit, readmeCompleted: readme }) ? (
                    <>
                      ...
                      <div>
                        <h4 className="text-xs font-bold text-white uppercase tracking-wider">Metrics Approved</h4>
                        <p className="text-[11px] text-slate-450 mt-1 leading-normal">
                          All mandatory evidence criteria met. Save this record to submit proof.
                        </p>
                      </div>
                    </>
                  ) : (
                    <>
                      ...
                      <div>
                        <h4 className="text-xs font-bold text-white uppercase tracking-wider">Verification Pending</h4>
                        <p className="text-[11px] text-slate-450 mt-1 leading-normal">
