  ...
  if (isBooting) {
    return (
      <div className="fixed inset-0 z-50 bg-navy-950 flex flex-col items-center justify-center text-center">
        ...
        <h2 className="text-xl font-bold text-white tracking-tight mb-3">Preparing XcelerateAI</h2>
        <p className="text-slate-400 text-[14px] flex items-center gap-2">
          Loading your course <Loader2 className="w-3.5 h-3.5 animate-spin" />
        </p>
      </div>
    );
  }

  if (curriculumMode === 'catalog') {
    return <Navigate to="/curricula" replace state={{ from: location.pathname }} />;
  }
  ...
  if (!hasImport) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-center max-w-md mx-auto space-y-4">
        ...
        <h2 className="text-2xl font-bold text-white tracking-tight">Course unavailable</h2>
        <p className="text-slate-400 text-sm leading-relaxed">
          A course must be configured before this section can be used.
        </p>
        <a href="/settings#advanced-settings" className="btn-primary mt-4">
  ...
  if (!isDataReady) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-center space-y-4">
        ...
        <h2 className="text-xl font-bold text-white tracking-tight">Preparing your course...</h2>
