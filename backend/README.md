...
Backend V1 verification remains deliberately modest and zero-cost. Server structural checks confirm only evidence shape and finalized metadata. One explicit PYAE Week 3 requirement can run advisory Python AST checks through a lazily loaded Pyodide Web Worker; learner source remains local and only the result plus SHA-256 fingerprint is sent to the API. Client checks are inspectable and not tamper-resistant. Server sandbox and AI rubric adapters remain unconfigured future extension points. Pyodide's first load is large and requires connectivity; browser caching may improve later runs, but first-use offline execution is not promised.
