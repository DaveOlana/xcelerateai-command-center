...
## Optional build-template validation

The `templates` field is optional on practical missions, projects, and supported week-level build entities. Its absence is valid and remains backward compatible.

When present, verify that:

- `templates` is an array;
- every entry is an object with a non-empty string `id`, `label`, and `content`;
- IDs are unique within that owning entity;
- stable machine-friendly IDs are used, such as `readme`, `env-example`, `api-docs`, or `bug-report`;
- content is plain-text scaffolding that does not complete the learner's task;
- a custom `readme` template overrides the bundled fallback without duplication;
- the fallback appears only when `filesToCreate` explicitly includes README or a non-empty `readmePrompt` exists;
- vague prose is not treated as a README requirement;
- `readmePrompt` guidance is preserved separately from copyable template content.

Templates remain curriculum content, so normal curriculum backup/import carries them automatically. Do not create a separate learner-progress or template-storage field.
