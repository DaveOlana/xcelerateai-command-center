...
# Optional Build Templates

Buildable entities may provide copyable starter scaffolds with an optional `templates` array. Templates belong to the practical mission, project, or supported week-level build entity they scaffold—not to Proof, notes, or learner progress.
...
Each entry requires the V1 canonical fields `id`, `label`, and `content`. IDs must be non-empty and unique within the owning entity. Use stable machine-friendly IDs such as `readme`, `env-example`, `api-docs`, or `bug-report`; labels remain learner-facing text.

Templates are optional and must scaffold rather than solve the learner's work. A curriculum-authored `readme` template overrides the application fallback. When explicit README work is required through `filesToCreate` or `readmePrompt` and no custom `readme` template exists, the application supplies its bundled README scaffold. No explicit README requirement means no manufactured fallback. `readmePrompt` remains project-specific guidance and complements, rather than replaces, the template.

Template content is copied exactly as authored. Do not pre-fill learner discoveries, challenges, lessons, features, project names, or completed solutions.
